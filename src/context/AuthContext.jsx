...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (nombre, email, password, rol) => {
    try {
      const response = await fetch("https://https://tourmatchterminar-1.onrender.com2.onrender.com/api/auth/registro", {
        method: "POST",
  ...
  const login = async (email, password) => {
    try {
      const response = await fetch("https://https://tourmatchterminar-1.onrender.com2.onrender.com/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
