...
const DashboardAdmin = () => {
  ...
  const cargarConductores = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/admin/conductores`, {
  ...
  const cargarTuristas = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/admin/turistas`, {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
      if (response.ok) {
        const data = await response.json();
...
// 💰 LÓGICA DE NEGOCIO: Calculamos la comisión SOLO de viajes terminados y cobrados
  ...
  const ejecutarEliminar = async (id, tipo) => {
    ...
    const url = tipo === 'reserva' ? `${API_URL}/api/admin/reservas/${id}` : `${API_URL}/api/admin/usuarios/${id}`;
    try {
  ...
  const ejecutarGuardarEdicion = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/admin/usuarios/${usuarioAEditar.id}`, {
        method: 'PUT',
        headers: {
