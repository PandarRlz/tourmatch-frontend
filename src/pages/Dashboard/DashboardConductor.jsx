...
const DashboardConductor = () => {
  ...
  const consultarVehiculo = useCallback(async () => {
    ...
    try {
      const response = await fetch("https://tourmatchterminar-2.onrender.com/api/vehiculos/mi-vehiculo", {
  ...
  const consultarViajesDisponibles = useCallback(async () => {
    ...
    try {
      const response = await fetch(`https://tourmatchterminar-2.onrender.com/api/reservas/disponibles?capacidad=4`, {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
  ...
  const consultarViajesActivos = useCallback(async () => {
    if (!user?.token) return;
    try {
      const response = await fetch(`https://tourmatchterminar-2.onrender.com/api/reservas/mis-viajes-conductor`, {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
  ...
  const handleAceptarViaje = async (id) => {
    if (!user?.token) return;
    try {
      const response = await fetch(`https://tourmatchterminar-2.onrender.com/api/reservas/${id}/aceptar`, {
  ...
  const handleFinalizarViaje = async (id, ganancia) => {
    if (!user?.token) return;
    try {
      const response = await fetch(`https://tourmatchterminar-2.onrender.com/api/reservas/${id}/finalizar`, {
        method: "PUT", headers: { "Authorization": `Bearer ${user.token}` }
      });
      if (response.ok) {
